...
/**
 * useMegaLeadForm — Gulf2Bay Softwash LP form submission hook.
 ...
 *   - The LP marks `qualified: true|false` + `disqualification_reason` so the
 *     backend can fire the `qualified_lead` Google Ads conversion only for
 *     qualified leads. Disqualified leads still land in CRM (Jobber) +
 *     service@gulf2baysoftwash.com so the client can review.
 */
  ...
  const submit = useCallback(
    async (
      ...
      const payload: SubmissionPayload = {
        ...
        source_provider: options.sourceProvider ?? "gulf2bay-softwash-landing",
        form_data: formData,
